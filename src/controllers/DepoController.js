...
module.exports = {
  async criar(req, res) {
    if (!req.userId) {
      return res.status(401).send({ error: 'faltando token' });
    }
    ...
    try {
      ...
      return res
        .status(400)
        .send({ error: `${e} Verifique os dados digitados` });
    }
  },
  ...
  async alterar(req, res) {
    ...
    try {
      let alterar = await Depoimento.findByIdAndUpdate(
        req.params.id,
        req.body,
        { new: true },
      );
  ...
  async deletar(req, res) {
    try {
      ...
      return res
        .status(400)
        .send({ error: `${e} Ocorreu um erro ao tentar excluir` });
    }
  },
  ...
  async getTrue(req, res) {
    try {
      const depoimentos = await Depoimento.find({ active: true })
        .sort('-createdAt')
        .populate({
          path: 'idusuario',
          select: 'nome avatar',
  ...
  async alterarActive(req, res) {
    ...
    try {
      ...
      } else await depo.updateOne({ active: false });
      return res.json({ ok: true });
    } catch (e) {
      return res
        .status(400)
